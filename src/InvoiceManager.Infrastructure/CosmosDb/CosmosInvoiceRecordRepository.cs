...
public sealed class CosmosInvoiceRecordRepository : IInvoiceRecordRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // statuses (Expected, RetrievalError, Retrieved, FreeAgentMatchExpected) are
        // all picked up - see docs/workflow-states.md. FreeAgentError is deliberately
        // NOT included yet: DueInvoiceProcessor does not yet know how to resume a
        // FreeAgentError record (it would re-run OneDrive reconciliation/retrieval
        // from scratch instead of resuming the FreeAgent stage) - that lands together
        // with the dispatch-loop rework that makes it safe to include. Terminal
